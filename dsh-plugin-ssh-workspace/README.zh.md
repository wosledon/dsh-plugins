...
> **状态：已装进 profile，等待重启宿主后验证真机行为。**
> 宿主半与客户端半都已写完，离线自检 **113 项断言**全过（含两次 SHA256 校验的
> 负向对照）。**尚未验证**的是真机行为：宿主半是否激活、工具是否被模型看到、
> SSH 是否真能连上。宿主半改动**必须重启宿主**才生效——Node ESM 缓存按解析路径
> 缓存，仅重新激活 Loader 行不会重载模块（实测：报错仍指向旧行号）。
