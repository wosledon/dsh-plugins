...
- **独立页面**：侧边栏面板图标（`sidebar.panellist`，id `token-usage`，order 30）
  ...
  **每页 20 行**，带一个「刷新」按钮。表格上方还有两张图：按模型占比的环形图，
  ...
  两天时画出来的仍然是整整 30 天（5 列 35 格），而不是孤零零两列。空格子表达的是
  **"这天没有用量"** —— 那正是热力图要传达的信息（哪天断过），不是谎报；窗口必须由
  ...
  （手打 2020 年只会得到一年宽的窗口，而不是横跨六年的网格）；夹过的值写回输入框
...
node scripts/verify-layout.mjs    # 全部通过：304 项布局与结构断言
...
（30 天窗口 = 5 列 = 35 格 + 5 个图例色块 = 40 个格子节点），而不是"大于 0"。
把 `renderHeatmap` 里那一步补窗去掉（负向对照 A）时，"3 天数据也铺满 5 列"的断言立刻红。

日期区间用同一套办法钉：默认窗口锚在**数据里最晚那天**（负向对照把锚点改成"今天"时套件变红）、
窗口**不按数据起点收缩**（负向对照 B 把 `from` 夹回数据最早那天时，"3 天数据也铺满 30 天"
...
以及"补满之后几何的跨度就是窗口"（2 天数据 + 30 天窗口 → 5 列 35 格，不补则只有 1 列）。
