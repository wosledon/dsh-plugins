...
window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    function renderHeatmap(points, t, options) {
       ...
       *
       * 取值必须用**该列第一个有日期的格子**（`cell !== null`），而**不能**用
       * "第一个有用量的格子"。
       *
       * 这里被真机推翻过两次，两次都是"不报错、只是标签错"：
       *
       *   1. 最初取"第一个非 null 格子"，而当时的格子数组里，窗口起点**之前**
       *      那几天也被补成了 0 值格子（真机数据里 09-13 那一列的前 5 天就是），
       *      于是 6 月的图顶着「5月」。
       *   2. 于是改成"第一个 `total > 0` 的格子"。这在只有零星几天有数据时**更糟**：
       *      一年窗口里只有一个月的列含用量，其余 11 个月的列全是 0，全都拿不到
       *      月份——真机截图里整个一年**只显示了一个「9月」**。
       *
       * 正确的判据是**日历**而不是用量：月份属于"这一列覆盖哪些日期"，
       * 与这两天有没有用过毫无关系。而当初第 1 条的病根（窗口外被补成 0）
       * 已由 `padRangeToDays` 只补窗口内 + 几何把窗口外标成 `null` 解决，
       * 所以现在 `cell !== null` 恰好就是"落在窗口内"，正是要的语义。
       */
      const monthOf = (week) => {
        const first = week.find((cell) => cell !== null && cell !== undefined) ?? null;
